...
const LanguageSwitcher: FC<Props> = ({ message, animation, animation2, className }) => {
    ...
    return (
        <Dropdown
            button={
                <button className="text-blue-600 dark:text-white uppercase h-5 w-5 
          rounded-md flex justify-center place-items-center font-bold text-xs pt-[1px]">
                    {locale}
                </button>
            }
        >
            <div className="absolute right-0 translate-x-14 flex min-w-32 w-32 !z-50 flex-col justify-start border dark:border-white/10 rounded-md bg-white bg-cover bg-no-repeat shadow-xl shadow-shadow-500 dark:!bg-[#242526] dark:text-white dark:shadow-none">
                {/* Option 1: English */}
                <button
