  ...
  return (
    <motion.nav
      ...
      <RenderCase renderIf={openNotification}>
        <SubmitPopup message={message} submit={logout} onClose={() => setOpenNotification(false)} />
      </RenderCase>
      ...
      <div className={` h-7 ${openApp ? "bg-white dark:bg-[#242526] shadow-[4px_-4px_10px_rgba(0,0,0,0.3)]" : "backdrop-blur-xl bg-white/80 dark:bg-[#242526]/50"} flex gap-4 justify-between transition-all duration-200 px-2 py-1 rounded-md`}>
        ...
        <RenderCase renderIf={pathname == "/menu"}>
          <div className={`text-md font-semibold text-blue-700 dark:text-white place-items-center w-1/3 justify-center hidden sm:flex`}>
