...
const ProductsMain = () => {
    ...
    const reloadData = useCallback(async () => {
        ...
        if (token) {
            const response = await ProductOp.getSpecialOffer({ token }, { currentPage });
    ...
    const submitCreateProduct = async () => {
        ...
        const createData: CreateProductInfo = {
            ...productData,
            ListPrice: typeof productData.ListPrice === 'string' ? parseFloat(productData.ListPrice) : productData.ListPrice,
            StandardCost: typeof productData.StandardCost === 'string' ? parseFloat(productData.StandardCost) : productData.StandardCost
        }

        const token = localStorage?.getItem("accessToken");
        const response = await ProductOp.createProductInfo({ token }, createData);
    ...
    const submitUpdateProduct = async (productID: string) => {
        ...
        const updateData = {
            ...productDetail,
            productID,
            ListPrice: typeof productDetail.ListPrice === 'string' ? parseFloat(productDetail.ListPrice) : productDetail.ListPrice,
            StandardCost: typeof productDetail.StandardCost === 'string' ? parseFloat(productDetail.StandardCost) : productDetail.StandardCost
        };
    ...
    const handleChange = (id: keyof ProductInfo, value: string | number) => {
        setProductDetail((prev) => ({ ...prev, [id]: value }));
    };

    const handleChange2 = (id: keyof CreateProductInfo, value: string | number) => {
